...
class SimulatedPerson(id:Int, times:Int) extends Actor {
  ...
  var updateProfileNum = 10


  
  // if 2% user delete friends
  var deleteFriendNum = Random.nextInt(10000)+1 // [1,10000]
  if(deleteFriendNum - 9980 > 0){
    deleteFriendNum = 1
  }

  var deletePostNum = 10
  ...
  def receive = {
    case "posted" => {
      ...
      if( postedNum >= (postNum_FirstBunch+postNum_SecondBunch-10) && deletionFinished == false){ // when all post are sent
...
object Main extends App {
  ...
  while(regDone == false) {
    ...
    ret match {
      case x: Int => if(x >= userNum*0.95){regDone = true}else{println(x)}
      case _ => {}
    }
    var i = 0L
    while(i != 95000L){
